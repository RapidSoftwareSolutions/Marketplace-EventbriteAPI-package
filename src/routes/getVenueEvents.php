...
$app->post('/api/EventbriteAPI/getVenueEvents', function ($request, $response, $args) {
    ...
    $query_str = $settings['api_url'] . 'venues/'.$post_data['args']['venueId'].'/events/';
    
    $body = [];
    if(!empty($post_data['args']['status'])) {
        $body['status'] = $post_data['args']['status'];
    }
    if(!empty($post_data['args']['order_by'])) {
        $body['orderBy'] = $post_data['args']['order_by'];
    }
