...
$app->post('/api/EventbriteAPI/getEventAccessCodes', function ($request, $response, $args) {
    ...
    try {
        ...
        if($rawBody->pagination->page_count != 1) {
            $pagin = $this->pager;
            $ret = $pagin->page($query_str, 2, $headers, 'access_codes');
            
            $merge = array_merge($all_data[0]->access_codes, $ret);
        
            $all_data[0]->access_codes = $merge;
        }
